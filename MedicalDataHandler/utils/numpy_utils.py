...
def numpy_roi_mask_generation(cols, rows, mask: np.array, matrix_points: np.array, geometric_type: str):
    """
...
def poly2mask(vertex_row_coords: np.array, vertex_col_coords: np.array, shape: tuple) -> np.array:
    """
...
def create_HU_to_RED_map(hu_values, red_values):
    """
    Creates a mapping function to convert Hounsfield Units (HU) to Relative Electron Density (RED) values.
    
    Args:
        hu_values (list): A list of HU values.
        red_values (list): A list of RED values.
    
    Returns:
        function: A linear interpolation function mapping HU to RED.
    """
    if not hu_values or not red_values:
        print(f"Warning: HU and RED values not provided. Ignoring the user input, and using default values as backup!")
        hu_values = _get_backup_hu_values()
        red_values = _get_backup_red_values()
    if not isinstance(hu_values, (list, tuple)) or not isinstance(red_values, (list, tuple)):
        print(f"Warning: HU and RED values must be provided as lists or tuples. Ignoring the user input, and using default values as backup!")
        hu_values = _get_backup_hu_values()
        red_values = _get_backup_red_values()
    if not len(hu_values) == len(red_values):
        print(f"Warning: HU and RED values must be of equal length. Ignoring the user input, and using default values as backup!")
        hu_values = _get_backup_hu_values()
        red_values = _get_backup_red_values()
    
    return interp1d(np.array(hu_values), np.array(red_values), kind="linear", bounds_error=False, fill_value="extrapolate")

def _get_backup_hu_values():
    """ Returns a list of HU values for the backup RED to HU mapping. """
    return [
...
def _get_backup_red_values():
    """ Returns a list of RED values for the backup RED to HU mapping. """
    return [
